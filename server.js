...
// ═══════════════════════════════════════════════════════════════════════════
// UPDATED: MSS/BOS CACHE - Handles new format with signalType
// ═══════════════════════════════════════════════════════════════════════════
...
function addSignal(signal) {
  cache.signals.unshift({
    ...signal,
    id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
    timestamp: Date.now()
  });
  
  if (cache.signals.length > 200) {
    cache.signals = cache.signals.slice(0, 200);
  }
  
  cache.dirty.signals = true;
}
